...
fun validar(objeto: Any): List<String> {
    ...
    objeto::class.members.forEach { member ->
        member.annotations.forEach { annotation ->
            ...
            when (annotation) {
                is Positivo -> if (valor !is Int || valor <= 0) msgs.add("O valor $valor não é um número positivo")
                is NaoVazio -> if (valor !is String || valor.trim().isEmpty()) msgs.add("O valor $valor está vazio")
            }
        }
    }
    return msgs;
